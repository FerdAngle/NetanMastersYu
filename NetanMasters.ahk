...
global formList := {}                                         ; Mainloop at bottom
global formDisplayList := ""
global TPhaseC := "After TP"
global isTransformed := false
...
global slotList := {}
global slotDisplayList := "" ;most recently added, idc theres duplicate code im NOT making another label/function 
...
HelpInfoGUI:
 ...
 -----------------------------------------
    ...
    ;bro make this MORE descriptive 
    Gui, 3:Show, Center, InfoHelp
return   

SaveSettings:
    ;MsgBox,0x40000,, % A_GuiEvent "And its length: " StrLen(A_GuiEvent)
    Gui, Submit, NoHide
    ;MsgBox,0x10,,% TPKeyEdit "has length" StrLen(TPKeyEdit)
    if !RegExMatch(XTimeEdit, "^\d*$") { ;permits ONLY digits and an empty field if the user wants to instant transform 
        ;MsgBox, 0x40030,, % "Enter a valid number from 0 onwards!"
...
MenuRenameHandler:  ; RENAMES THE CURRENT FORM SELECTED
    Gui, Submit, NoHide
    if  (modifyingForms) {
        oldName := formEdit
        component := "Form"
    ...
    WinSet, AlwaysOnTop, Off, NetanMastersYu ; make a function of this in the future maybe not, 
    Gui, +Disabled
    HotkeyStates("Off")
    ...
    if (modifyingForms){
        if (InStr(formDisplayList, newName . "|")){
            MsgBox,0x40000, Moron Detected, % "DUPLICATES ARE DISALLOWED", 1
...
MenuDeleteHandler:   ; DELETES THE CURRENT FORM SELECTED
    Gui, Submit, NoHide
    if (modifyingForms)
        selected := formEdit
    if (modifyingSlots)
...
MainLoop:   ;----------------------------------------------  MAIN -------------------------------------------------------------------------------------
    ...
    ; IT ratio 0.7525 , now death watcher
    ; as for when to transform while getting beaten up, maybe use a timer, i doubt pixels will bode well, have your alt m2 instead of m1 chaining?
    ; accomodate for FSSJ
    ; sanitise hotkey entries , prevent backspaces/spacebars
    ; HP Pixel for zenkaing CE4202
    ; HP Pixel for death watcher to resume the MainLoop D0A600

    ; also add in frieza support, I literally main it 
return 

; for the far future, add a preset option
...
RespawnDetector:
    ...
    if !WinExist("Roblox")
        return
    WinGetPos, winX, winY, winW, winH, Roblox
    HPPixelYellow := 0xCBA300                         ; old pixel 0xCEA500, CEA200 CFA300
...
; TO DO LIST: Test the effectiveness of HP Zenkai and consider if ones ki bar should be lowered too...  
;  


;c::                                    ;---Debugging tool, useful for finding ratios between certain images to the window resolution 
   ; CoordMode, Mouse, Window
  ;  WinGetPos, WinX, WinY, WinW, WinH, Roblox 
 ;   MouseGetPos, mx1, my1
 ;   ToolTip % "x:" mx1  "`ny:" my1 "`nrobloxY:" WinH "`nrobloxX:" WinW "`nRatioY:" (my1/WinH) "`nRatioX:" (mx1/WinW) 
;return 

;l::
  ;  Shades := 0 
 ;  SetTimer, PixelSearchingTest, 200
;return 

PixelSearchingTest:  
    MsgBox,0x40000,, % "Running pixel search", 1
    TargetPixel := 0xD90005 ; combatcheck
    WinGetPos, winX, winY, winW, winH, Roblox
    CoordMode, Pixel, Window
    PixelSearch, px, py
    , winX, winY + (winH * 0.15)
    ...
    , Fast RGB 
    if (ErrorLevel = 0){
        SetTimer, PixelSearchingTest, Off 
        MouseMove, px, py
        MsgBox,0x40000,, % "X:" px  "`nY:" py 
    } else {
        Shades += 1 
    }
return 
...
; create an array list for CombatLog, RespawnDetector for pixels to use cuz I have no idea why they change randomly ;nah
; test out combat logger and finalise whether a reset or a rejoin is more appropriate. ()
; if a corpse death happens, just switch servers bro, its not worthwhile to code against it
